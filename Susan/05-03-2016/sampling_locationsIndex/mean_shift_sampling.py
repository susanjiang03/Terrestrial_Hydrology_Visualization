...
def mean_shiftf_clustering(in_filename, list_of_variable):
	...
	data_matrix = []
	#filter out the sample index 
	fo = open("sampleLocations.txt")
	r = fo.readline()
	indexList = r.split(" ")
	#print indexList
	indexList = map(int, indexList[:-1])
	#print indexList


	for row in data_as_list:
		new = []
		if int(row[0]) in indexList:
	...
	plt.title('%s  variables Estimated number of clusters: %d' % (in_filename, n_clusters_))
	plt.show()


datafile = "1980-03-01.csv"
list_of_variable =  [5,20]
...
mean_shiftf_clustering("1980-03-01.csv",list_of_variable)
