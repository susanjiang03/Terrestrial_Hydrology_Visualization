...
def mean_shiftf_clustering(in_filename, list_of_variable):
	...
	ms = MeanShift(andwidth=bandwidth, bin_seeding=True)
	ms.fit(X)
	labels = ms.labels_
	...
	plt.xlabe(list_of_variable[0])
	plt.ylabel(list_of_variable[1])
	plt.title(in_filename)
	plt.title('Estimated number of clusters: %d' % n_clusters_)
	plt.show()


datafile = "./data/date_2001-12-01.csv"
