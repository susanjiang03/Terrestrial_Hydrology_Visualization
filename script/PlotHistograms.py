...
'''
get a list of data of variable [index] from in_fileName on the number of cluster label
@in_fileName, the csv file 
@index: which column in the file
@cluster_label_column_name
@num_cluster_label, if None, get data for all the varialbe , if lagrer than the number of clusters, throw error. 
'''
def get_histogram_data_by_index_clusters(in_fileName, index, cluster_label_column_name, num_cluster):
	header = get_header(in_fileName)
	return_list_of_data = []
	if num_cluster_label == None:
		print "Get histogram data for all the variables"
        data_as_list = get_data_as_list(in_fileName,range(4,38))
    else:
    	if cluster_label_column_name not in header:
            return  "Error.No clusters label data yet."
        else:
	    	n_clusters_ = int(cluster_label_column_name.split('C')[-1])
	    	if num_cluster_label > n_clusters_ -1:
		    	return "Error. The maximun input num_cluster for this %s is %d"%(cluster_label_column_name,%num_cluster)
		    else:
		    	data_as_list = filter_by_index_value(in_fileName,header.index(cluster_label_column_name),num_cluster)
    for row in data_as_list:
		return_list_of_data.append(int(row[index]))
    return return_list_of_data
